// api/contact.js
// Receives the "Get Started" lead form from the GrabNjoy Solution website
...
async function sendEmail({ to, subject, html }) {
  ...
  try {
    const r = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: 'GrabNjoy Solution <support@grabnjoy.com>', to: toList, subject, html }),
    });
    return r.ok;
...
module.exports = async function handler(req, res) {
  ...
  const notifyTo = (process.env.LEADS_NOTIFY_EMAILS || 'grabNjoySolution@gmail.com').split(',').map(e => e.trim());
...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#FCFBF8;">
  <div style="background:#fff;border-radius:12px;padding:28px;box-shadow:0 2px 8px rgba(5,32,77,0.08);">
    ...
    <p style="font-size:13px;color:#3C4F6E;margin:0 0 20px;">Submitted via grabnjoy.com</p>
